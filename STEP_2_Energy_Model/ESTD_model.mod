...
param solar_area >= 0; # Maximum land available for PV deployement [km2]
...
var Share_Freight_Road, >= share_freight_road_min, <= share_freight_road_max; # %_Road: Ratio [0; 1] Road transport over total freight transport
var Share_Freight_Boat, >= share_freight_boat_min, <= share_freight_boat_max; # %_Boat: Ratio [0; 1] boat transport over total freight transport
...
var Shares_Mobility_Freight {TECHNOLOGIES_OF_END_USES_CATEGORY["MOBILITY_FREIGHT"]} >=0; # %_Freight [-]: Constant share of passenger mobility
...
subject to extra_grid:
	F ["GRID"] = 1 + (c_grid_extra / c_inv["GRID"]) * (F ["WIND_ONSHORE"] + F ["WIND_OFFSHORE"] + F ["PV"]) / (f_max ["WIND_ONSHORE"] + f_max ["WIND_OFFSHORE"] + f_max ["PV"]);
...
subject to Freight_shares :
	Share_Freight_Train + Share_Freight_Road + Share_Freight_Boat = 1; # =1 should work... But don't know why it doesn't
...
# [Eq. 30] Compute the equivalent size of V2G batteries based on the share of V2G, the amount of cars and the battery capacity per EVs technology
...
# [Eq. 33] Calculation of max heat demand in DHN (1st constrain required to linearised the max function)
...
subject to max_elec_import {h in HOURS, td in TYPICAL_DAYS}:
	F_t ["ELECTRICITY", h, td] <= import_capacity; 
	
# [Eq. 39] Limit surface area for solar
subject to solar_area_limited250km2 :
	F["PV"]/0.2367+(F["DEC_SOLAR"]+F["DHN_SOLAR"])/0.2857 <= solar_area;
