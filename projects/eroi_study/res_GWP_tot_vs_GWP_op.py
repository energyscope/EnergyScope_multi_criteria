...
def plot_asset_capacities_by_tech(df_assets: pd.DataFrame, pdf: str, user_data:str, dir_plot:str):
    ...
    plot_stacked_bar(df_data=df_elec_tech, ylabel='[GW]', ylim=90, pdf_name=dir_plot + '/f-elec-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_heat_low_DEC, ylabel='[GW]', ylim=35, pdf_name=dir_plot + '/f-heat-low-DEC-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_heat_low_DHN, ylabel='[GW]', ylim=28, pdf_name=dir_plot + '/f-heat-low-DHN-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_heat_high_T, ylabel='[GW]', ylim=25,pdf_name=dir_plot + '/f-heat-high-T-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_synthetic_fuels, ylabel='[GW]', ylim=20,  pdf_name=dir_plot + '/f-synthetic-fuels-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_elec_storage, ylabel='[GWh]', ylim=150, pdf_name=dir_plot + '/f-elec-storage-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_thermal_storage, ylabel='[GWh]', ylim=1300, pdf_name=dir_plot + '/f-thermal-storage-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_other_storage, ylabel='[GWh]', ylim=45000,  pdf_name=dir_plot + '/f-other-storage-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_mob_private, ylabel='[GW]', ylim=220, pdf_name=dir_plot + '/f-mob-private-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_mob_public, ylabel='[GW]', ylim=40,  pdf_name=dir_plot + '/f-mob-public-' + pdf + '.pdf')
    plot_stacked_bar(df_data=df_mob_freight, ylabel='[GW]', ylim=95,   pdf_name=dir_plot + '/f-freight-' + pdf + '.pdf')
...
def fec_plots(df_fec_data: pd.DataFrame, pdf: str, dir_plot: str):
    ...
    FEC PLOTS
    """
    plot_stacked_bar(df_data=df_fec_data.transpose(), ylabel='(TWh)', ylim=520,
                     pdf_name=dir_plot + '/fec-details-' + pdf + '.pdf')
    # FEC detailed by EUD
    heat_list = ['HEAT_HIGH_T', 'HEAT_LOW_T_DHN', 'HEAT_LOW_T_DECEN']
    plot_stacked_bar(df_data=df_fec_data.loc[heat_list].transpose(), ylabel='(TWh)', ylim=205,
    ...
    plot_stacked_bar(df_data=df_fec_data.loc[mob_list].transpose(), ylabel='(TWh)', ylim=100,
                     pdf_name=dir_plot + '/fec-details-mob-' + pdf + '.pdf')

    non_energy_list = ['HVC', 'AMMONIA', 'METHANOL', 'ELECTRICITY']
    plot_stacked_bar(df_data=df_fec_data.loc[non_energy_list].transpose(), ylabel='(TWh)', ylim=200,
...
def primary_energy_plots(df_EI_1: pd.DataFrame, df_EI_2: pd.DataFrame, pdf_1: str, pdf_2: str, dir_plot: str):
    ...
    plot_stacked_bar(df_data=df_1, ylabel='(TWh)', ylim=520, pdf_name=dir_plot + '/EI-RE-' + pdf_1 + '.pdf')
    ...
    plot_stacked_bar(df_data=df_2[l_new], ylabel='(TWh)', ylim=520, pdf_name=dir_plot + '/EI-RE-' + pdf_2 + '.pdf')
...
if __name__ == '__main__':
    ...
    plot_data(df_data_1=df_res_1['EROI'], df_data_2=df_res_2['EROI'], label_1=label1, label_2=label2, pdf_name=dir_plot+'/eroi_'+ str(domestic_RE_share)+'.pdf', x_index=[i for i in range_val], ylim=[2.5, 10], ylabel='EROI [-]')
    plot_data(df_data_1=df_res_1['FEC'], df_data_2=df_res_2['FEC'], label_1=label1, label_2=label2, pdf_name=dir_plot+'/fec_'+ str(domestic_RE_share)+'.pdf', x_index=[i for i in range_val], ylim=[300, 480], ylabel='FEC [TWh/y]')
    plot_data(df_data_1=df_res_1['Einv'], df_data_2=df_res_2['Einv'], label_1=label1, label_2=label2, pdf_name=dir_plot+'/einv_'+ str(domestic_RE_share)+'.pdf', x_index=[i for i in range_val], ylim=[30, 180], ylabel=r'$Einv{tot}$ [TWh/y]')
    plot_data(df_data_1=df_GWP_1.sum(axis=1), df_data_2=df_GWP_2.sum(axis=1), label_1=label1, label_2=label2, pdf_name=dir_plot+'/gwp_'+ str(domestic_RE_share)+'.pdf', x_index=[i for i in range_val], ylim=[0, 105], ylabel=r'$GWP_{tot}$ [MtC02/y]')
    plot_data(df_data_1=df_GWP_1['GWP_cons'], df_data_2=df_GWP_2['GWP_cons'], label_1=label1, label_2=label2, pdf_name=dir_plot+'/gwp_const_'+ str(domestic_RE_share)+'.pdf', x_index=[i for i in range_val], ylim=[0, 10], ylabel=r'$GWP_{const}$ [MtC02/y]')
    plot_data(df_data_1=df_GWP_1['GWP_op'], df_data_2=df_GWP_2['GWP_op'], label_1=label1, label_2=label2, pdf_name=dir_plot+'/gwp_op_'+ str(domestic_RE_share)+'.pdf', x_index=[i for i in range_val], ylim=[0, 100], ylabel=r'$GWP_{op}$ [MtC02/y]')
    ...
    plot_stacked_bar(df_data=df_EI_cat_1.transpose(), ylabel='(TWh)', ylim=520, pdf_name=dir_plot+'/EI-details-'+pdf_1+'.pdf')
    plot_stacked_bar(df_data=df_EI_cat_2.transpose(), ylabel='(TWh)', ylim=520, pdf_name=dir_plot+'/EI-details-'+pdf_2+'.pdf')
    ...
    plot_stacked_bar(df_data=df_Einv_RES_cat_1.transpose(), ylabel='(TWh)', ylim=160, pdf_name=dir_plot+'/einv-res-'+pdf_1+'.pdf')
    plot_stacked_bar(df_data=df_Einv_RES_cat_2.transpose(), ylabel='(TWh)', ylim=160, pdf_name=dir_plot+'/einv-res-'+pdf_2+'.pdf')

    # Einv_op classed by RESOURCES
    df_1 = retrieve_non_zero_val(df=df_Einv_op_1.transpose())
    plot_stacked_bar(df_data=df_1, ylabel='(TWh)', ylim=160, pdf_name=dir_plot+'/einv-res-details-'+pdf_1+'.pdf')
    df_2 = retrieve_non_zero_val(df=df_Einv_op_2.transpose())
    plot_stacked_bar(df_data=df_2, ylabel='(TWh)', ylim=160, pdf_name=dir_plot+'/einv-res-details-'+pdf_2+'.pdf')
