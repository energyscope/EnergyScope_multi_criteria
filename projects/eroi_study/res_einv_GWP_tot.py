...
def fec_plots(df_fec_data: pd.DataFrame, pdf: str, dir_plot: str):
    ...
    plot_stacked_bar(df_data=df_fec_data.transpose(), xlabel=xlabel,  ylabel='[TWh]', ylim=520,
    ...
    df_fec_data_heat = rename_columns(df=df_fec_data_heat, old_col=heat_list,
                                      new_col=['Heat high T.', 'Heat low T. DHN', 'Heat low T. DEC'])
    plot_stacked_bar(df_data=df_fec_data_heat, xlabel=xlabel,  ylabel='FEC [TWh]', ylim=205,
    ...
    df_fec_data_mob = rename_columns(df=df_fec_data_mob, old_col=mob_list,
                                     new_col=['Mob. public', 'Mob. private', 'Mob. freight rail',
                                              'Mob. freight boat', 'Mob. freight road'])
    plot_stacked_bar(df_data=df_fec_data_mob, xlabel=xlabel, ylabel='FEC [TWh]', ylim=100,
    ...
    df_fec_data_non_energy = rename_columns(df=df_fec_data_non_energy, old_col=non_energy_list,
                                            new_col=['HVC', 'Ammonia', 'Methanol', 'Electricity'])
    plot_stacked_bar(df_data=df_fec_data_non_energy, xlabel=xlabel, ylabel='FEC [TWh]', ylim=200,
...
if __name__ == '__main__':
    ...
    df_assets_ = res_assets_capacity(range_val=range_val, dir_name=dir_name)
    ...
    for p in range(100, 0, -5):
        ...
        print('GWP_tot %.1f [MtCO2-eq./y]: EROI %.1f offshore %.1f [GW] onshore %.1f [GW] PV %.1f [GW] NUC %.1f [GW]' %
              (df_GWP.sum(axis=1).loc[p], df_res['EROI'].loc[p], df_assets_[p].loc['WIND_OFFSHORE'],
               df_assets_[p].loc['WIND_ONSHORE'], df_assets_[p].loc['PV'], df_assets_[p]['NUCLEAR']))
    ...
    #
    #
    # df_layer_gas_5 = pd.read_csv(dir_name + '/run_5/' + "/output/hourly_data/layer_GAS.csv",
    #                              index_col=0).dropna(axis=1)
    ...
    ####################################################################################################################
    if config_name_file == 'config_2035_2_GW_nuc':
        dir_plot_ = 'einv_GWP_tot_2_GW_nuc_' + str(domestic_RE_share)
    elif config_name_file == 'config_2035_5_6_GW_nuc':
        dir_plot_ = 'einv_GWP_tot_5_6_GW_nuc_' + str(domestic_RE_share)
    else:
        dir_plot_ = 'einv_GWP_tot_' + str(domestic_RE_share)

    make_dir(cwd+'/export/')
    make_dir(cwd+'/export/'+dir_plot_+'/')
    if config_name_file == 'config_2035_2_GW_nuc':
        dir_plot_ = cwd + '/export/einv_GWP_tot_2_GW_nuc_' + str(domestic_RE_share)
    elif config_name_file == 'config_2035_5_6_GW_nuc':
        dir_plot_ = cwd + '/export/einv_GWP_tot_5_6_GW_nuc_' + str(domestic_RE_share)
    else:
        dir_plot_ = cwd + '/export/einv_GWP_tot_' + str(domestic_RE_share)
    pdf_ = 'gwp-tot-' + str(domestic_RE_share)
    ...
    plot_one_serie(df_data=df_cost.sum(axis=1), label='Cost',
                   pdf_name=dir_plot_ + '/cost_' + str(domestic_RE_share) + '.pdf',
                   x_index=x_gwp_tot_index, ylim=[40, 100], ylabel='[bEUR/y]')
    plot_one_serie(df_data=df_res['EROI'], label='EROI',
                   pdf_name=dir_plot_ + '/eroi_custom_' + str(domestic_RE_share) + '.pdf',
    ...
    plot_one_serie(df_data=df_EI.drop(columns=['Subcategory']).transpose().sum(axis=1), label='Primary energy',
                   pdf_name=dir_plot_ + '/EI_tot_' + str(domestic_RE_share) + '.pdf',
                   x_index=x_gwp_tot_index, ylim=[350, 550], ylabel='[TWh/y]')

    ####################################################################################################################
    # FEC
    df_fec_details.columns = np.round(x_gwp_tot_index, 1)
    fec_plots(df_fec_data=df_fec_details, pdf=pdf_, dir_plot=dir_plot_)

    ####################################################################################################################
    # PRIMARY ENERGY

    df_EI_cat = rename_columns(df=df_EI_cat.transpose(),
    ...
    plot_stacked_bar(df_data=df_EI_cat.drop(columns=['Export']),
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Primary energy [TWh]', ylim=600,
                     pdf_name=dir_plot_+'/EI-categories-'+pdf_+'.pdf')
    ...
    df_EI_RES_RE = rename_columns(df=df_EI_RES_RE,
    ...
    plot_stacked_bar(df_data=df_EI_RES_RE[['Ammonia-RE', 'Methanol-RE', 'Gas-RE', 'Wet biomass', 'Wood',
                                           'Hydro', 'Solar', 'Wind']],
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Primary energy [TWh]', ylim=530,
                     pdf_name=dir_plot_ + '/EI-RE-' + pdf_ + '.pdf', colors=colors)
    ...
    df_EI_RES_non_RE = rename_columns(df=df_EI_RES_non_RE,
    ...
    plot_stacked_bar(df_data=df_EI_RES_non_RE, xlabel='Yearly emissions limit [MtCO2-eq./y]',
                     ylabel='Primary energy [TWh]', ylim=350, pdf_name=dir_plot_ + '/EI-non-RE-' + pdf_ + '.pdf',
                     colors=colors)
    ...
    plot_stacked_bar(df_data=df_Einv_RES_cat.transpose(), xlabel='Yearly emissions limit [MtCO2-eq./y]',
                     ylabel='Operation energy [TWh]', ylim=160, pdf_name=dir_plot_+'/einv-res-'+pdf_+'.pdf')
    ...
    plot_stacked_bar(df_data=df_einv_op_filtered,
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Operation energy [TWh]', ylim=160,
                     pdf_name=dir_plot_+'/einv-op-details-'+pdf_+'.pdf')
    ...
    df_einv_op_RE_filtered = rename_columns(df=df_einv_op_RE_filtered,
                                            old_col=['AMMONIA_RE', 'METHANOL_RE', 'GAS_RE', 'WET_BIOMASS', 'WOOD'],
                                            new_col=['Ammonia-RE', 'Methanol-RE', 'Gas-RE', 'Wet biomass', 'Wood'])
    plot_stacked_bar(df_data=df_einv_op_RE_filtered,
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Operation energy [TWh]', ylim=160,
                     pdf_name=dir_plot_+'/einv-op-re-res-'+pdf_+'.pdf')
    ...
    df_einv_constr_elec_tech = rename_columns(df=df_einv_constr_elec_tech,
                                              old_col=['CCGT', 'PV', 'WIND_ONSHORE', 'WIND_OFFSHORE'],
                                              new_col=['CCGT', 'PV', 'Onshore wind', 'Offshore wind'])
    plot_stacked_bar(df_data=df_einv_constr_elec_tech,
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Construction energy [TWh]', ylim=ymax,
                     pdf_name=dir_plot_+'/einv_const-elec-'+pdf_+'.pdf')
    ...
    df_einv_constr_mob_tech = rename_columns(df=df_einv_constr_mob_tech,
                                             old_col=['CAR_NG', 'CAR_BEV', 'TRUCK_ELEC', 'TRUCK_NG'],
                                             new_col=['Gas car', 'Elec. car', 'Elec. truck', 'Gas truck'])
    plot_stacked_bar(df_data=df_einv_constr_mob_tech,
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Construction energy [TWh]', ylim=ymax,
                     pdf_name=dir_plot_+'/einv_const-mob-'+pdf_+'.pdf')
    ...
    plot_stacked_bar(df_data=df_gwp_op_filtered,
                     xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Operation emissions [MtCO2-eq./y]',
                     ylim=100, pdf_name=dir_plot_ + '/gwp_op-breakdown-' + pdf_ + '.pdf')

    ####################################################################################################################
    # Plot assets installed capacities with stacked bars
    df_assets_.columns = np.round(x_gwp_tot_index, 1)
    plot_asset_capacities_by_tech(df_assets=df_assets_, pdf=pdf_, user_data=config['user_data'], dir_plot=dir_plot_,
                                  xlabel='Yearly emissions limit [MtCO2-eq./y]')

    ####################################################################################################################
    # Plot assets installed capacities with lines
    # plot_asset_capacities_by_tech_line(df_assets=df_assets, pdf=pdf, user_data=config['user_data'],
    #                                    dir_plot=dir_plot, xlabel='Yearly emissions [MtC02]')

    def plot_asset_capacities_by_tech_line(df_assets: pd.DataFrame, pdf: str, user_data: str, dir_plot: str,
                                           xlabel: str = 'p (%)'):
        """
        ...
        df_heat_low_dhn = retrieve_non_zero_val(
            df=df_assets.loc[tech_by_subcategory['Heat low temperature centralised']].transpose())
        df_heat_high_t = retrieve_non_zero_val(
            df=df_assets.loc[tech_by_subcategory['Heat high temperature']].transpose())
        df_synthetic_fuels = retrieve_non_zero_val(df=df_assets.loc[tech_by_subcategory['Synthetic fuels']].transpose())
        df_elec_storage = retrieve_non_zero_val(
            df=df_assets.loc[tech_by_subcategory['Electricity storage']].transpose())
        df_thermal_storage = retrieve_non_zero_val(df=df_assets.loc[tech_by_subcategory['Thermal storage']].transpose())
        ...
        plt.plot(x_index, df_elec_tech['WIND_ONSHORE'].values + df_elec_tech['WIND_ONSHORE'].values, '-Dg', linewidth=3,
                 markersize=5, label='Wind')
        plt.plot(x_index, df_elec_tech['CCGT'].values, '-Dk', linewidth=3, markersize=5, label='CCGT')
        plt.plot(x_index, df_heat_high_t['IND_COGEN_GAS'].values + df_heat_low_dhn['DHN_COGEN_GAS'].values, '--P',
                 color='orange', linewidth=3, markersize=5, label='IND. +DHN gas CHP')
        plt.plot(x_index, df_heat_low_dhn['DHN_COGEN_BIO_HYDROLYSIS'].values, '--P', color='brown', linewidth=3,
        ...
        # plt.plot(x_index, df_heat_low_dec['DEC_ADVCOGEN_GAS'].values,
        #          '-D', linewidth=3, markersize=5, label='DEC_ADVCOGEN_GAS')
        plt.plot(x_index, df_heat_low_dec['DEC_BOILER_GAS'].values,
        ...
        plt.plot(x_index, df_heat_low_dhn['DHN_BOILER_GAS'].values,
                 '-D', linewidth=3, markersize=5, label='Gas boilers')
        plt.plot(x_index, df_heat_low_dhn['DHN_COGEN_GAS'].values, '-D', linewidth=3, markersize=5, label='Gas CHP')
        plt.plot(x_index, df_heat_low_dhn['DHN_COGEN_BIO_HYDROLYSIS'].values,
        ...
        plt.figure()
        plt.plot(x_index, df_heat_high_t['IND_BOILER_WASTE'].values,
        ...
        plt.plot(x_index, df_heat_high_t['IND_DIRECT_ELEC'].values,
        ...
        plt.plot(x_index, df_synthetic_fuels['METHANE_TO_METHANOL'].values,
                 '-D', linewidth=3, markersize=5, label='Methane to methanol')
        plt.plot(x_index, df_synthetic_fuels['BIOMASS_TO_METHANOL'].values,
                 '-D', linewidth=3, markersize=5, label='Biomass methanolation')
        plt.plot(x_index, df_synthetic_fuels['HABER_BOSCH'].values,
                 '-D', linewidth=3, markersize=5, label='Haber Bosch')
        plt.plot(x_index, df_synthetic_fuels['METHANOL_TO_HVC'].values,
        ...
        plt.figure()
        plt.plot(x_index, df_thermal_storage['TS_DEC_HP_ELEC'].values,
                 '-D', linewidth=3, markersize=5, label='TS DEC HP')
        plt.plot(x_index, df_thermal_storage['TS_DEC_ADVCOGEN_GAS'].values,
                 '-D', linewidth=3, markersize=5, label='TS DEC FC CHP gas')
        plt.plot(x_index, df_thermal_storage['TS_DEC_BOILER_GAS'].values,
        ...
        plt.plot(x_index, df_other_storage['METHANOL_STORAGE'].values,
        ...
        # plt.plot(x_index, df_thermal_storage['TS_DEC_BOILER_GAS'].values,
        #          '-D', linewidth=3, markersize=5, label='TS DEC boiler gas')
        # plt.plot(x_index, df_thermal_storage['TS_DHN_SEASONAL'].values,
        #          '-D', linewidth=3, markersize=5, label='TS DHN')
        # plt.plot(x_index, df_other_storage['GAS_STORAGE'].values, '-P', linewidth=3, markersize=5, label='Gas')
        # plt.plot(x_index, df_other_storage['AMMONIA_STORAGE'].values,
