    ...
    for batch in [1, 2, 3, 4, 5, 6]:
        ...
        for sample_i in range(0, N):
            ...
            for key in einv_constr_keys:
                all_data['Technologies'].at[key[1], key[0]] = \
                    df_einv_constr_samples.loc[sample_i][key[0] + '-' + key[1]]
            for key in c_pt_keys:
                all_data['Time_series'][key[1]] = \
                    all_data['Time_series'][key[1]] * df_c_pt_samples.loc[sample_i][key[0] + '-' + key[1]]\
                    / all_data['Time_series'][key[1]].mean()
            for param in ['import_capacity', 'i_rate']:
                config['system_limits'][param] = df_other_samples.loc[sample_i]['other-' + param]
            for share_max in ['share_mobility_public_max', 'share_freight_train_max',
                              'share_freight_boat_max', 'share_heat_dhn_max']:
                config['system_limits']['technologie_shares'][share_max] = \
                    df_other_samples.loc[sample_i]['other-' + share_max]
            for loss_network in ['ELECTRICITY', 'HEAT_LOW_T_DHN']:
                config['system_limits']['loss_network'][loss_network] = \
                    df_other_samples.loc[sample_i]['other-' + loss_network]
