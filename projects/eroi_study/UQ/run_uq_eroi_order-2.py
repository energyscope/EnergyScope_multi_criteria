...
if __name__ == '__main__':
    ...
    df_samples.loc[2945] = df_samples3.loc[len(df_samples)] # specific case for run 2945 where the computation reaches the timelimit
    M_var = len(df_samples.columns)
    n_samples = 2 *  (M_var + 2) * (M_var + 1) / 2

    param_list = json.load(open('data_samples/param_list_order_2.json'))
    ...
    for sample_i in range(0, len(df_samples)):
        ...
        # for share_max in ['share_mobility_public_max', 'share_freight_train_max', 'share_freight_boat_max',
        #                   'share_heat_dhn_max']:
        for share_max in ['share_mobility_public_max', 'share_freight_train_max',
       'share_freight_boat_max']:
