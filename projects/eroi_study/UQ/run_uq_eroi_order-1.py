...
gwp_tot_max = 100300 # ktCO2/y -> constraint on the GWP_tot, 100300, 85400, 56900, 42700, 28500, 19000
new = True
if __name__ == '__main__':
    ...
    if new:
        path_samples = 'samples-order-1-new.csv'
    else:
        path_samples = 'samples-order-1.csv'

    for batch in [1, 2, 3, 4, 5, 6]:
        dir_name = 'einv_uq_'+str(batch) + '_gwp_' + str(gwp_tot_max)

        # Load samples generated by RHEIA
        df_samples = pd.read_csv('data_samples/'+path_samples, index_col=0)
        ...
        param_list = json.load(open('data_samples/param_list.json'))
        demand_keys = [key.split('-') for key in param_list['demand']]
        df_eud_samples = df_samples_batch[param_list['demand']].copy()
        avail_keys = [key.split('-') for key in param_list['avail']]
        df_avail_samples = df_samples_batch[param_list['avail']].copy()
        einv_op_keys = [key.split('-') for key in param_list['einv_op']]
        df_einv_op_samples = df_samples_batch[param_list['einv_op']].copy()
        f_max_keys = [key.split('-') for key in param_list['f_max']]
        df_f_max_samples = df_samples_batch[param_list['f_max']].copy()
        einv_constr_keys = [key.split('-') for key in param_list['einv_constr']]
        df_einv_constr_samples = df_samples_batch[param_list['einv_constr']].copy()
        c_pt_keys = [key.split('-') for key in param_list['c_pt']]
        df_c_pt_samples = df_samples_batch[param_list['c_pt']].copy()
        df_other_samples = df_samples_batch[param_list['other']].copy()

        # Note: batch 6 is only composed of additional runs to be used in case of outliers in run 1 to 5
        if batch == 6:
            N = 20
        else:
            N = n_samples
        ...
        for sample_i in range(0, N):
            ...
            for key in c_pt_keys:
                all_data['Time_series'][key[1]] = all_data['Time_series'][key[1]] * df_c_pt_samples.loc[sample_i][key[0] + '-' + key[1]] / all_data['Time_series'][key[1]].mean()
            ...
            es.print_estd(out_path=f"{config['temp_dir']}/ESTD_data.dat", data=all_data, system_limits=config["system_limits"])
            es.print_12td(out_path=f"{config['temp_dir']}/ESTD_12TD.dat", time_series=all_data['Time_series'], step1_output_path=config["step1_output"])

            # Running EnergyScope
            cs = f"{config['case_studies_dir']}/{dir_name+'/sample_'+str(sample_i)}"
            es.run_step2_new(cs, config['AMPL_path'], config['options'], [f"{config['ES_path']}/ESTD_model.mod"], [f"{config['temp_dir']}/ESTD_data.dat", f"{config['temp_dir']}/ESTD_12TD.dat"], config['temp_dir'], dump_res_only=True)
