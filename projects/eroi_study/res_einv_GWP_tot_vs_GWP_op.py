...
def fec_plots(df_fec_data: pd.DataFrame, pdf: str, dir_plot: str):
    ...
    plot_stacked_bar(df_data=df_fec_data.loc[heat_list].transpose(), xlabel=xlabel,  ylabel='[TWh]', ylim=205,
    ...
    plot_stacked_bar(df_data=df_fec_data.loc[mob_list].transpose(), xlabel=xlabel, ylabel='[TWh]', ylim=100,
                     pdf_name=dir_plot + '/fec-details-mob-' + pdf + '.pdf')

    non_energy_list = ['HVC', 'AMMONIA', 'METHANOL', 'ELECTRICITY']
    plot_stacked_bar(df_data=df_fec_data.loc[non_energy_list].transpose(), xlabel=xlabel, ylabel='[TWh]', ylim=200,
...
def primary_energy_plots(df_ei_1: pd.DataFrame, df_ei_2: pd.DataFrame, pdf_1: str, pdf_2: str, dir_plot: str):
    ...
    plot_stacked_bar(df_data=df_1, xlabel='p (%)', ylabel='(TWh)', ylim=520,
    ...
    plot_stacked_bar(df_data=df_2[l_new], xlabel='p (%)', ylabel='(TWh)', ylim=520,
    ...
    plot_stacked_bar(df_data=df_1, xlabel='p (%)',  ylabel='(TWh)', ylim=400,
    ...
    plot_stacked_bar(df_data=df_2, xlabel='p (%)',  ylabel='(TWh)', ylim=400,
...
if __name__ == '__main__':
    ...
    df_einv_const_2 = retrieve_einv_const_by_categories(range_val=range_val, all_data=all_data, dir_name=dir_2,
                                                        user_data=config['user_data'])
    df_assets_1 = res_assets_capacity(range_val=range_val, dir_name=dir_1)
    df_assets_2 = res_assets_capacity(range_val=range_val, dir_name=dir_2)
    ...
    ####################################################################################################################
    # FEC
    fec_plots(df_fec_data=df_fec_details_1, pdf=pdf_1_, dir_plot=dir_plot_)
    fec_plots(df_fec_data=df_fec_details_2, pdf=pdf_2_, dir_plot=dir_plot_)

    ####################################################################################################################
    # PRIMARY ENERGY
    # RESOURCES subcategories: Other non-renewable, Fossil fuel, Biofuel, Non-biomass (WIND, SOLAR, HYDRO, ...)
    plot_stacked_bar(df_data=df_EI_cat_1.transpose(), xlabel='p (%)',  ylabel='(TWh)', ylim=520,
                     pdf_name=dir_plot_+'/EI-details-'+pdf_1_+'.pdf')
    plot_stacked_bar(df_data=df_EI_cat_2.transpose(), xlabel='p (%)',  ylabel='(TWh)', ylim=520,
    ...
    plot_stacked_bar(df_data=df_einv_res_cat_1.transpose(), xlabel='p (%)',  ylabel='(TWh)', ylim=160,
                     pdf_name=dir_plot_+'/einv-res-'+pdf_1_+'.pdf')
    plot_stacked_bar(df_data=df_einv_res_cat_2.transpose(), xlabel='p (%)', ylabel='(TWh)', ylim=160,
                     pdf_name=dir_plot_+'/einv-res-'+pdf_2_+'.pdf')

    # Einv_op classed by RESOURCES
    df_1_ = retrieve_non_zero_val(df=df_einv_op_1.transpose())
    plot_stacked_bar(df_data=df_1_, xlabel='p (%)', ylabel='(TWh)', ylim=160,
                     pdf_name=dir_plot_+'/einv-res-details-'+pdf_1_+'.pdf')
    df_2_ = retrieve_non_zero_val(df=df_einv_op_2.transpose())
    plot_stacked_bar(df_data=df_2_, xlabel='p (%)', ylabel='(TWh)', ylim=160,
                     pdf_name=dir_plot_+'/einv-res-details-'+pdf_2_+'.pdf')

    # 2. Einv_const by TECHNOLOGIES categories: electricity, mobility, heat, ...
    plot_stacked_bar(df_data=df_einv_tech_cat_1.transpose(), xlabel='p (%)', ylabel='(TWh)', ylim=35,
                     pdf_name=dir_plot_+'/einv-tech-'+pdf_1_+'.pdf')
    plot_stacked_bar(df_data=df_einv_tech_cat_2.transpose(), xlabel='p (%)', ylabel='(TWh)', ylim=35,
    ...
    for cat_ in df_einv_const_1.keys():
        ...
        if len(df_einv_const_1[cat_].columns) > 0:
            plot_stacked_bar(df_data=df_einv_const_1[cat_], xlabel='p (%)', ylabel='(TWh)', ylim=ymax,
                             pdf_name=dir_plot_+'/einv_const-'+cat_+'-'+pdf_1_+'.pdf')
        if len(df_einv_const_2[cat_].columns) > 0:
            plot_stacked_bar(df_data=df_einv_const_2[cat_], xlabel='p (%)', ylabel='(TWh)', ylim=ymax,
                             pdf_name=dir_plot_+'/einv_const-'+cat_+'-'+pdf_2_+'.pdf')

    ####################################################################################################################
    # PLot assets installed capacities
    plot_asset_capacities_by_tech(df_assets=df_assets_1, pdf=pdf_1_, user_data=config['user_data'], dir_plot=dir_plot_)
    plot_asset_capacities_by_tech(df_assets=df_assets_2, pdf=pdf_2_, user_data=config['user_data'], dir_plot=dir_plot_)
