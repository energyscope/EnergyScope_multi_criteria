...
def loop_eroi_computation(range_val, dir_name: str, gwp_op_ini: float, config: dict):
    """
    Minimize the Einv for several GWP_tot <= gwp_limit, with gwp_limit = p* GWP_op a GHG emission target.
    ...
    for gwp_tot_max, cs_name in zip(np.asarray([i for i in range_val]) * gwp_op_ini / 100,
        ...
        # Running EnergyScope -> INCLUDING GREY EMISSIONS (constrain in GWP_tot):
...
if __name__ == '__main__':
    ...
    config_['system_limits']['re_be_share_primary'] = DOMESTIC_RE_SHARE

    if config_name_file == 'config_2035_2_GW_nuc':
        dir_name_ = 'einv_GWP_tot_2_GW_nuc_' + str(int(100 * DOMESTIC_RE_SHARE))
    elif config_name_file == 'config_2035_5_6_GW_nuc':
        dir_name_ = 'einv_GWP_tot_5_6_GW_nuc_' + str(int(100 * DOMESTIC_RE_SHARE))
    else:
        dir_name_ = 'einv_GWP_tot_' + str(int(100 * DOMESTIC_RE_SHARE))
    ...
    # Running ES-TD
    mod_fns_ = [f"{config_['ES_path']}/ESTD_model.mod"]
    ...
    # the initial GWP_op is always defined without nuclear!
    ...
    loop_eroi_computation(range_val=range_val_, dir_name=dir_name_, gwp_op_ini=gwp_op_ini_, config=config_)
