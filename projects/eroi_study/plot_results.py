...
def get_FEC_from_sankey(case_study_dir: str, col:str):
    ...
    Return the final energy consumption (FEC) (TWh) by use sector: 'Non-energy demand', 'Loss DHN',
     'Heat LT DHN', 'Exp & Loss', 'Mob public', 'Heat LT Dec', 'Elec demand', 'Freight', 'Mob priv', 'Heat HT'
    """
...
def post_treatment(range_val, dir_name: str, GWP_op_ini: float, all_data: dict):
    ...
    for gwp_limit, cs_name in zip(np.asarray([i for i in range_val]) * GWP_op_ini / 100,
        ...
        print('Case %s Einv %.1f Eout %.1f EROI "final" %.2f GWP_op %.2f (MtC02eq)' % (cs_name, einv_temp, ef_temp_tot,
                                                                                       eroi_temp, gwp_limit))
...
def compute_einv_details(cs: str, energyscope_dir: str, all_data: dict):
    """
    Compute the Einv by RESOURCES and TECHNOLOGIES, it details the breakdown by
    subcategories of RESOURCES and categories of TECHNOLOGIES.
...
if __name__ == '__main__':
    ...
    df_inv_res_by_subcat, df_inv_tech_by_cat = compute_einv_details(cs=cs, energyscope_dir=config['energyscope_dir'],
                                                                    all_data=all_data)
    ...
    unwanted_RES = ['BIOETHANOL', 'BIODIESEL', 'GAS_RE', 'H2_RE', 'AMMONIA_RE',
                    'METHANOL_RE', 'ELEC_EXPORT', 'CO2_EMISSIONS']
    RESOURCES_bis = [res for res in RESOURCES if res not in unwanted_RES]
    non_mobility_RES = ['URANIUM', 'RES_WIND', 'RES_SOLAR', 'RES_HYDRO', 'RES_GEO', 'CO2_ATM',
                        'CO2_INDUSTRY', 'CO2_CAPTURED', 'CO2_EMISSIONS']
    ...
    # GWhe -> convert in thermal GWh? -> compute the ratio of RE resources and fuel
    # resources to the get the efficiency (100% for SOLAR, WIND, ...; NUC 1/2.7027 etc)
    FEC_ELECTRICITY = df_eud['ELECTRICITY']
    ...
    # # Min Einv
    # # s.t. GWP_tot <= p * GWP_op^i with p a percentage and GWP_op^i the value obtained
    # # by Min Einv without conqtraint on GWP_tot
    # # -----------------------------------------------
    #
    # # GWP op
    # dir_name_0 = 're_be_0'
    ...
    # df_Eout_0, df_eroi_0, df_inv_0, df_einv_res_by_subcat_0, df_einv_tech_by_cat_0 = \
    #   post_treatment(range_val=range_val_0, dir_name=dir_name_0, GWP_op_ini=GWP_op_ini_0, all_data=all_data)
    # df_Eout_30, df_eroi_30, df_inv_30, df_einv_res_by_subcat_30, df_einv_tech_by_cat_30 = \
    #   post_treatment(range_val=range_val_30, dir_name=dir_name_30, GWP_op_ini=GWP_op_ini_30, all_data=all_data)
    #
    # ####################################################################################################################
    # # -----------------------------------------------
    # # PLOT
    ...
    # # Plot EROI vs GWP
    # plt.figure()
    # plt.plot([i for i in range_val_0], df_eroi_0.values,
    #          '-Dk', linewidth=3, markersize=10, label='EROIf: RE share 0%')
    # plt.plot([i for i in range_val_30], df_eroi_30.values,
    #          '-Db', linewidth=3, markersize=10, label='EROIf: RE share 30%')
    ...
    # plt.figure()
    # plt.plot([i for i in range_val_0], df_Eout_0.sum().values,
    #          '-Db', linewidth=3, markersize=10, label='Eout: RE share 0%')
    # plt.plot([i for i in range_val_0], df_inv_0.values, '-Dr', linewidth=3, markersize=10, label='Einv: RE share 0%')
    # plt.plot([i for i in range_val_30], df_Eout_30.sum().values,
    #          '-Dg', linewidth=3, markersize=10, label='Eout: RE share 30%')
    # plt.plot([i for i in range_val_30], df_inv_30.values,
    #          '-Dy', linewidth=3, markersize=10, label='Einv: RE share 30%')
    # plt.gca().invert_xaxis()
    # plt.xticks([i for i in range_val])
    # plt.ylabel('(TWh)')
    # plt.xlabel('GWP op (%)')
    ...
    #     plt.plot(list(df_einv_res_by_subcat_0.columns), df_einv_res_by_subcat_0.loc[subcat].values,
    #              'D', linewidth=3, markersize=5, label=subcat)
    # plt.gca().invert_xaxis()
    # plt.xticks([i for i in range_val])
    ...
    #     plt.plot(list(df_einv_tech_by_cat_0.columns), df_einv_tech_by_cat_0.loc[subcat].values,
    #                   'D', linewidth=3, markersize=5, label=subcat)
    # plt.gca().invert_xaxis()
    # plt.xticks([i for i in range_val])
