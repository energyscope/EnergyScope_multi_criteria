...
if __name__ == '__main__':
    ...
    plot_stacked_bar(df_data=df_EI_cat.drop(index=['Export']).transpose(), xlabel='Yearly emissions limit [MtCO2-eq./y]',  ylabel='Primary energy [TWh]', ylim=600, pdf_name=dir_plot+'/EI-categories-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=df_EI_RES_RE[['Ammonia-RE', 'Methanol-RE', 'Gas-RE','Wet biomass', 'Wood', 'Hydro', 'Solar', 'Wind', 'H2-RE', 'Biodiesel']], xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Primary energy [TWh]', ylim=530, pdf_name=dir_plot + '/EI-RE-' + pdf + '.pdf', colors=colors)
    ...
    plot_stacked_bar(df_data=df_EI_RES_non_RE[['NG', 'Elec. import', 'Ammonia', 'Waste', 'LFO', 'Coal']], xlabel='Yearly emissions limit [MtCO2-eq./y]', ylabel='Primary energy [TWh]', ylim=350, pdf_name=dir_plot + '/EI-non-RE-' + pdf + '.pdf', colors=colors)
    ...
    plot_stacked_bar(df_data=df_Einv_RES_cat.transpose(), xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=160, pdf_name=dir_plot+'/einv-res-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=df_einv_op_filtered, xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=160, pdf_name=dir_plot+'/einv-op-details-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=df_einv_op_RE_filtered, xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=160, pdf_name=dir_plot+'/einv-op-re-res-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=df_einv_op_non_RE_filtered, xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=30, pdf_name=dir_plot+'/einv-op-non-re-res-'+pdf+'.pdf')

    # 2. Einv_const by TECHNOLOGIES categories: electricity, mobility, heat, ...
    df_Einv_TECH_cat.columns =np.round(x_gwp_tot_index, 1)
    plot_stacked_bar(df_data=df_Einv_TECH_cat.drop(index=['Infrastructure']).transpose(), xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=35, pdf_name=dir_plot+'/einv-tech-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=Einv_const_dict['Electricity'][elec_tech], xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=ymax, pdf_name=dir_plot+'/einv_const-elec-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=Einv_const_dict['Mobility'][mobility_tech], xlabel='GWP total [MtCO2-eq./y]', ylabel='[TWh]', ylim=ymax, pdf_name=dir_plot+'/einv_const-mob-'+pdf+'.pdf')
    ...
    plot_stacked_bar(df_data=df_gwp_const_concat, xlabel='p [%]', ylabel='[MtCO2-eq./y]', ylim=9, pdf_name=dir_plot + '/gwp_const-breakdown-' + pdf + '.pdf')
    ...
    plot_stacked_bar(df_data=df_gwp_op_filtered, xlabel='p [%]', ylabel='[MtCO2-eq./y]', ylim=100, pdf_name=dir_plot + '/gwp_op-breakdown-' + pdf + '.pdf')
    ...
    plot_asset_capacities_by_tech(df_assets=df_assets, pdf=pdf, user_data=config['user_data'], dir_plot=dir_plot, xlabel='[MtCO2-eq./y]')
